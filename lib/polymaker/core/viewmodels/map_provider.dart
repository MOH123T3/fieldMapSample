...
class MapProvider extends ChangeNotifier {
  ...
  void onTapMap(LatLng _location,
      {TrackingMode? mode = TrackingMode.LINEAR}) async {
    if (isEditMode == true) {
  ...
  Future<String> calculateDistance(
    ...
    if (distance < 1) {
      double selectedMeter = double.parse(distance.toStringAsFixed(3)) * 1000;
      totalMeters.add(selectedMeter.toString().split(".")[0]);
